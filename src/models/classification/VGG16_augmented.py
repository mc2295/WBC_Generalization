...
def train_model(train_dl, valid_dl, model, num_epochs=40, criterion= CrossEntropyLoss()):
    optimizer = SGD(model.parameters(), lr=0.01, momentum=0.9)
    total_step = len(train_dl)

    with open('results_VGG16_augmented.csv', 'w') as f:
    ...
    for epoch in range(num_epochs):
        ...
        with open('results_VGG16_augmented.csv', 'a') as f:
            writer = csv.writer(f, delimiter=';')
            writer.writerow([loss.item(), 100 * correct_train / total_train, [np.round(100 * acc_train[c]/max(total_per_class_train[c],1),decimals=1) for c in range(21)], '',''])
...
torch.save(model, 'models/VGG16_model_augmented')
