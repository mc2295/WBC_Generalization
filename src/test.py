...
model = torch.load('../models/Mixed_sources_trained/Barcelona_Saint_Antoine/resnet/resnet101_4_transform3')
workflow = Workflow('../')
transform = True
source_train = ['barcelona', 'saint_antoine']
size = [50,50]

lr = workflow.find_learning_rate(model, source, size = size, transform = transform)
print(lr)

## expected: SuggestedLRs(valley=2.0892961401841603e-05), SuggestedLRs(valley=0.00010964782268274575)
...
transform = True
size = [32]
b = workflow.show_batch(source, transform = transform, siamese_head = False, size = size)
print(b.shape)
#" expected torch.Size([32, 3, 224, 224])

b = workflow.show_batch(source, transform = transform, siamese_head = True, size = size)
print(b[0].shape)
#" expected torch.Size([32, 3, 224, 224])

b = workflow.show_batch(source_train, transform = False, siamese_head = True, size = [32, 32])
print(b[0].shape)

workflow = Workflow('../')
source_train = ['barcelona', 'rabin']
lr = 0.0001
size = [50, 50]
siamese_head = True
epoch_nb = 1
...
siamese_head = True
model2 = workflow.train_model(source_train, epoch_nb, siamese_head,  lr, batchsize = 8, size = size, transform = transform, weights = weights)
...
balanced = False
preds_to_look_at = None
model2 = workflow.fine_tune( model, source_tuning, batchsize, epoch_nb, lr, size = size, transform = transform, head_fine_tune = True, balanced = balanced, preds_to_look_at = preds_to_look_at)
...
balanced = False
preds_to_look_at = 'best_preds'
model2 = workflow.fine_tune( model, source_tuning, batchsize, epoch_nb, lr, size = size, transform = transform, head_fine_tune = True, balanced = balanced, preds_to_look_at = preds_to_look_at)



preds_to_look_at = 'worst_preds'
model2 = workflow.fine_tune( model, source_tuning, batchsize, epoch_nb, lr, size = size, transform = transform, head_fine_tune = True, balanced = balanced, preds_to_look_at = preds_to_look_at)


model = torch.load('../models/Mixed_sources_trained/Barcelona_Saint_Antoine/siamese8_transform3')

balanced = False
preds_to_look_at = None
model2 = workflow.fine_tune( model, source_tuning, batchsize, epoch_nb, lr, size = [100], transform = transform, head_fine_tune = True, balanced = balanced, preds_to_look_at = preds_to_look_at)
...
model = torch.load('../models/Mixed_sources_trained/Barcelona_Saint_Antoine/resnet/resnet101_4_transform3')
y = workflow.show_confusion_matrix(model, ['rabin'], batchsize = 32, size = [100,0,0,0], transform = True, plot_matrix = True, normalize = True)
...
data = workflow.show_flattened_images_in_2D(64, source, transform = transform)

model = torch.load('../models/Mixed_sources_trained/Barcelona_Saint_Antoine/siamese8_transform3')
df_embeddings = workflow.show_embeddings_2D(model, source, 't-SNE', display_classes = True, batchsize = 32,test_set = False, transform = True, size = [50,50])

model = torch.load('../models/Mixed_sources_trained/Barcelona_Saint_Antoine/resnet/resnet101_4_transform3')
df_embeddings = workflow.show_embeddings_2D(model, source, 't-SNE', display_classes = True, batchsize = 32,test_set = False, transform = True, size = [50,50])

group = workflow.show_images_from_scatter_plot_region(df_embeddings, ['lymphocyte'], [-60,60], [-60,60], transform = transform)

workflow = Workflow('../')
source = ['rabin']
workflow.show_distribution_of_prediction(model,source, batchsize = 16, test_set = True, transform = True, size = [500,500])

model = torch.load('../models/Mixed_sources_trained/Barcelona_Saint_Antoine/resnet/resnet101_4_transform3')
workflow.show_predictions_images(model, ['matek'], batchsize = 32, test_set = True, transform = True, size = [50], preds_to_look_at = None)

workflow.show_predictions_images(model, ['rabin'], batchsize = 32, test_set = True, transform = True, size = [500], preds_to_look_at = 'best_preds')

model = torch.load('../models/Mixed_sources_trained/Barcelona_Saint_Antoine/siamese8_transform3')
y =  workflow.label_with_siamese(model, ['rabin'], [10], ['saint_antoine'], [50], list_labels_cat, transform = True, batchsize = 8, plot_matrix = True, normalize = True)

print(y)

workflow = Workflow('../')
file_report_name = 'report/optim_fine_tune_test.csv'
model_name = 'resnet101_4_transform3'
source = ['rabin']
fine_tune_size = [32]
head_fine_tune = False
balanced = True
...
workflow.optimize_fine_tune(source, fine_tune_size, balanced, files_to_look_at, transform, preds_to_look_at, list_labels_cat, batchsize, model_name, file_report_name, head_fine_tune, k_fold = k_fold)


workflow = Workflow('../')
file_report_name = 'report/fine_tune_results_test.csv'
model_name = 'resnet101_4_transform3'
nb_of_iterations = 2
lr = [1e-7, 1e-2]
head_fine_tune = False
...
batchsize = 16
epoch_nb = 1
balanced_fine_tune = False
transform = True
preds_to_look_at = None


workflow.report_results_accuracy(file_report_name, nb_of_iterations,source, lr, head_fine_tune, epoch_nb, batchsize, model_name, transform = transform,balanced_fine_tune =  balanced_fine_tune, preds_to_look_at = preds_to_look_at)


model_name = 'resnet101_4_transform3'
workflow = Workflow('../')

source = ['barcelona', 'saint_antoine']
batchsize = 16
test_set = True
transform = True
size = [30,30]
preds_to_look_at = None
entry_path = '../'
file_report_name = 'report/cluster_evaluation_test.csv'
workflow.report_results_clusters(model_name, file_report_name, source, transform, size, test_set = True, batchsize = 16, preds_to_look_at = None)
...
for k in [32]:
    ...
    workflow.report_results_clusters(model_name, file_report_name, source, transform, size, test_set = test_set, batchsize = batchsize, preds_to_look_at = preds_to_look_at, balanced=balanced, show_graph = True, fine_tune = fine_tune, lr = lr, fine_tune_size = fine_tune_size, epoch_nb = epoch_nb)
